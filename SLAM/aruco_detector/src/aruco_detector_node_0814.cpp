...
ros::Publisher cam_pose_pub;     // camera pose in marker frame
ros::Publisher robot_pose_pub;   // robot pose in marker frame
ros::Publisher yaw_robot_marker_pub; // yaw of marker in robot(base) frame
...
cv::Vec3d cam_rpy_deg(0.32,0.0,0.24);          // 카메라 장착 RPY(deg)
...
static void rotMatToQuat(const cv::Mat& R, double& qx, double& qy, double& qz, double& qw)
{
    // R는 3x3, CV_64F
...
static inline double yaw_from_R_zyx(const cv::Mat& R)
{
    // ZYX yaw = atan2(R10, R00)
    return std::atan2(R.at<double>(1,0), R.at<double>(0,0)) * 180.0 / CV_PI;
}
...
void imageCallback(const sensor_msgs::ImageConstPtr& msg)
{
    ...
    cv::Mat R_total = R_cb * R_off; // R_B_C
    cv::Mat T_base_cam = (cv::Mat_<double>(3,1) << cam_xyz[0], cam_xyz[1], cam_xyz[2]); // t_B_C
    ...
    if (!ids.empty()) {
        ...
        for (size_t i = 0; i < ids.size(); ++i) {
            ...
            // --------- (A) 기본 카메라 기준 yaw/pitch 추출(표시용/기존 퍼블) ---------
            ...
            // --------- (B) 변환 행렬 구성 ----------
            // T_C->M
            cv::Mat R_C_M; cv::Rodrigues(rvecs[i], R_C_M);
            cv::Mat t_C_M = (cv::Mat_<double>(3,1) << tvecs[i][0], tvecs[i][1], tvecs[i][2]);

            // T_B->C
            cv::Mat R_B_C = R_total;
            cv::Mat t_B_C = T_base_cam;

            // ** 로봇 기준에서 본 마커 **  T_B->M = T_B->C * T_C->M
            cv::Mat R_B_M = R_B_C * R_C_M;
            cv::Mat t_B_M = R_B_C * t_C_M + t_B_C;

            // ** 마커 기준에서 본 카메라 **  T_M->C = (T_C->M)^-1
            cv::Mat R_M_C = R_C_M.t();
            cv::Mat t_M_C = -R_M_C * t_C_M;

            // ** 마커 기준에서 본 로봇 **   T_M->B = (T_B->M)^-1
            cv::Mat R_M_B = R_B_M.t();
            cv::Mat t_M_B = -R_M_B * t_B_M;

            // --------- (C) 로봇 기준 거리/오프셋 퍼블리시(기존 토픽 유지) ----------
            const double Xb = t_B_M.at<double>(0); // 전방(+)
            const double Yb = t_B_M.at<double>(1); // 좌(+)
            const double Zb = t_B_M.at<double>(2); // 위(+)

            std_msgs::Float64 m;
            m.data = Xb; z_pub.publish(m); // /aruco/pose_z
            m.data = Yb; x_pub.publish(m); // /aruco/pose_x

            // 로봇 기준에서 본 마커의 yaw (컨트롤에 쓰기 편함)
            double yaw_b_m_deg = yaw_from_R_zyx(R_B_M);
            std_msgs::Float64 yaw_rm; yaw_rm.data = yaw_b_m_deg;
            yaw_robot_marker_pub.publish(yaw_rm); // /aruco/yaw_robot_marker

            // --------- (D) PoseStamped 퍼블리시 (marker frame) ----------
            // 1) camera pose in marker frame (T_M->C)
            {
                double qx,qy,qz,qw; rotMatToQuat(R_M_C, qx,qy,qz,qw);
                geometry_msgs::PoseStamped ps;
                ps.header.stamp = msg->header.stamp;
                ps.header.frame_id = "aruco_marker";  // 필요시 "aruco_marker_<id>"
                ps.pose.position.x = t_M_C.at<double>(0);
                ps.pose.position.y = t_M_C.at<double>(1);
                ps.pose.position.z = t_M_C.at<double>(2);
                ps.pose.orientation.x = qx; ps.pose.orientation.y = qy;
                ps.pose.orientation.z = qz; ps.pose.orientation.w = qw;
                cam_pose_pub.publish(ps);
            }
            // 2) robot pose in marker frame (T_M->B)
            {
                double qx,qy,qz,qw; rotMatToQuat(R_M_B, qx,qy,qz,qw);
                geometry_msgs::PoseStamped ps;
                ps.header.stamp = msg->header.stamp;
                ps.header.frame_id = "aruco_marker";  // 필요시 "aruco_marker_<id>"
                ps.pose.position.x = t_M_B.at<double>(0);
                ps.pose.position.y = t_M_B.at<double>(1);
                ps.pose.position.z = t_M_B.at<double>(2);
                ps.pose.orientation.x = qx; ps.pose.orientation.y = qy;
                ps.pose.orientation.z = qz; ps.pose.orientation.w = qw;
                robot_pose_pub.publish(ps);
            }
            ...
            ROS_INFO("ID:%d | base Pos: [X=%.3f, Y=%.3f, Z=%.3f] | cam tvec:[%.3f, %.3f, %.3f] | Rot_cam(R,P,Y)=[%.1f, %.1f, %.1f] | yaw_b_m=%.1f",
                     ids[i], Xb, Yb, Zb, tvecs[i][0], tvecs[i][1], tvecs[i][2],
                     disp_roll, disp_pitch, disp_yaw, yaw_b_m_deg);
        }
    }
...
int main(int argc, char** argv)
{
    ...
    cam_pose_pub       = nh.advertise<geometry_msgs::PoseStamped>("aruco/camera_pose", 10);
    robot_pose_pub     = nh.advertise<geometry_msgs::PoseStamped>("aruco/robot_pose", 10);
    yaw_robot_marker_pub = nh.advertise<std_msgs::Float64>("aruco/yaw_robot_marker", 10);

    ROS_INFO_STREAM("ArUco Detector Node (cam->robot transform + 3 poses). "
