...
ros::Publisher yaw_pub;        // 카메라 기준 yaw (원하면 유지)
ros::Publisher yaw_b_m_pub;    // ★ 로봇(base) 기준 마커 yaw
...
// optical frame → base_link 고정 회전 사용 여부
bool    use_optical_to_base = true;
// 카메라 장착 오프셋 (base_link 기준)
cv::Vec3d cam_rpy_deg(0.0,0.0,0.0);   // roll, pitch, yaw (deg)
cv::Vec3d cam_xyz(0.0,0.0,0.0);       // x,y,z (m)
...
void imageCallback(const sensor_msgs::ImageConstPtr& msg)
{
    ...
    double disp_yaw_b_m = 0;                                  // ★ base 기준 마커 yaw
    ...
    if (!ids.empty()) {
        ...
        for (size_t i = 0; i < ids.size(); ++i) {
            ...
            // ★ base 기준 마커 yaw 계산: R_bm = R_bc * R_cm  (R_cm: cam→marker)
            cv::Mat R_cm;
            cv::Rodrigues(rvecs[i], R_cm);  // cam→marker 회전
            cv::Mat R_bm = R_total * R_cm;  // base→marker 회전

            double yaw_b_m = std::atan2(R_bm.at<double>(1,0), R_bm.at<double>(0,0)) * 180.0 / CV_PI;
            // 퍼블리시(원하면 사용)
            std_msgs::Float64 ybmsg; ybmsg.data = yaw_b_m; yaw_b_m_pub.publish(ybmsg);
            ...
            if (i == 0) {
                ...
                disp_pos_y   = Yb;
                disp_pos_z   = Zb;
                disp_yaw_b_m = yaw_b_m;
            }

            ROS_INFO("ID:%d | base Pos: [X=%.3f, Y=%.3f, Z=%.3f] | cam tvec:[%.3f, %.3f, %.3f] | cam RPY=[%.1f, %.1f, %.1f] | yaw_b(m)=%.1f",
                     ids[i], Xb, Yb, Zb, tvecs[i][0], tvecs[i][1], tvecs[i][2],
                     disp_roll, disp_pitch, disp_yaw, yaw_b_m);
        }
    }
    ...
    // Rot 대신 base기준 마커 yaw 표시
    std::snprintf(line2, sizeof(line2), "Yaw_b(m): %.1f deg", disp_yaw_b_m);
...
int main(int argc, char** argv)
{
    ...
    yaw_pub     = nh.advertise<std_msgs::Float64>("aruco/yaw",      10);  // 카메라 기준 yaw (옵션)
    yaw_b_m_pub = nh.advertise<std_msgs::Float64>("aruco/yaw_b_m",  10);  // ★ base 기준 마커 yaw
