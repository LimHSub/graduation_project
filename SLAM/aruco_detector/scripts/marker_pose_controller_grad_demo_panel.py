...
class MarkerPoseControllerPanel:
    def __init__(self):
        ...
        self.topic_mission = rospy.get_param("~topic_mission", "/panel_mission")
        self.topic_done = rospy.get_param("~topic_done", "/camera_mission_done")

        # left_panel만 추종
        self.target_class_name = rospy.get_param("~target_class_name", "left_panel")
        self.conf_thresh = float(rospy.get_param("~conf_thresh", 0.50))
        self.depth_patch = int(rospy.get_param("~depth_patch", 5))
        self.view_image = bool(rospy.get_param("~view_image", False))

        # 접근 제어 파라미터
        self.stop_distance_m = float(rospy.get_param("~stop_distance_m", 0.40))
        ...
        # PWM
        self.turn_pwm = int(rospy.get_param("~turn_pwm", 6))
        self.forward_pwm = int(rospy.get_param("~forward_pwm", 17))
        self.forward_slow_pwm = int(rospy.get_param("~forward_slow_pwm", 6))

        self.debug_image_topic = rospy.get_param("~debug_image_topic", "/marker_pose_controller/debug_image")
        ...
        rospy.loginfo("==== marker_pose_controller(left_panel 40cm) ====")
        ...
        rospy.loginfo("turn_pwm           : %d", self.turn_pwm)
        rospy.loginfo("forward_pwm        : %d", self.forward_pwm)
        rospy.loginfo("forward_slow_pwm   : %d", self.forward_slow_pwm)
        ...
        self.pub_doc_pwm = rospy.Publisher("/doc_pwm_cmd", Int16MultiArray, queue_size=1)
        self.pub_doc_cmd_char = rospy.Publisher("/doc_cmd_char", String, queue_size=1)
        self.pub_done = rospy.Publisher(self.topic_done, Int32, queue_size=1)
        self.pub_docking_done = rospy.Publisher("/docking_done", Bool, queue_size=1)
    ...
    def color_cb(self, msg):
        ...
        self.control_approach(err_x, depth_m)
    ...
    def control_approach(self, err_x, depth_m):
        if depth_m <= 0.0:
        ...
        # 중앙 정렬이 먼저
        if err_x > self.center_tolerance_px:
            # 오른쪽에 보이면 우회전: 좌측 +, 우측 -
            self.send_pwm(self.turn_pwm, -self.turn_pwm)
            return
        elif err_x < -self.center_tolerance_px:
            # 왼쪽에 보이면 좌회전: 좌측 -, 우측 +
            self.send_pwm(-self.turn_pwm, self.turn_pwm)
            return

        if depth_m > 0.60:
            self.send_pwm(self.forward_pwm, self.forward_pwm)
        else:
            self.send_pwm(self.forward_slow_pwm, self.forward_slow_pwm)
