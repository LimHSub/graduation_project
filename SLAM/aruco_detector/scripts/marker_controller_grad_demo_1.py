...
class MarkerPoseControllerGradDemo:
    ...
    def __init__(self):
        ...
        # ===== APPROACH 시작 전 사전 후진 추가 =====
        # marker_start_2는 /camera_mission_marker = +N 이므로 val > 0에서만 적용됨
        # marker_start_3는 /camera_mission_marker = -N 이므로 기존 RETREAT 로직 그대로 사용
        ...
        rospy.loginfo(
            ...
            "target_yaw=%.3f, direct_pwm=%s, pre_backward=%s, pre_backward_pwm=[%d,%d], pre_backward_duration=%.2f)",
            self.control_hz,
            self.angle_threshold,
            self.yaw_threshold,
            self.target_yaw,
            str(self.use_direct_pwm),
            str(self.use_pre_backward_on_approach),
            self.pre_backward_l,
            self.pre_backward_r,
    ...
    def run_control(self):
        ...
        # -------------------------
        # PRE_BACKWARD
        # marker_start_2 계열(+N, APPROACH) 시작 전에만 수행
        # 마커가 보이는지와 관계없이 3초간 후진 후 APPROACH로 넘어감
        # -------------------------
        if self.mode == self.MODE_PRE_BACKWARD:
            ...
            self.mode_pub.publish("APPROACH")
            rospy.loginfo(
                "[marker_ctrl] PRE_BACKWARD done -> APPROACH marker id=%s",
