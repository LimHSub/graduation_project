...
class MarkerPoseControllerPanel:
    def __init__(self):
        ...
        self.publish_debug_image_enabled = bool(
            rospy.get_param("~publish_debug_image", False)
        )

        # OpenCV 창 또는 디버그 이미지 publish가 필요할 때만
        # 프레임 복사 및 시각화 그리기 작업을 수행한다.
        self.debug_visualization_enabled = (
            self.view_image or self.publish_debug_image_enabled
        )
        ...
        if align_x_threshold is not None:
            try:
                ...
                self.center_tolerance_px = int(
                    rospy.get_param("~center_tolerance_px", 40)
                )
        else:
            self.center_tolerance_ratio = None
            self.center_tolerance_px = int(
        ...
        self.search_timeout_sec = float(
            rospy.get_param("~search_timeout_sec", 2.0)
        )
        self.control_hz = float(rospy.get_param("~control_hz", 10.0))

        # depth valid range
        self.max_valid_depth_m = float(
            rospy.get_param("~max_valid_depth_m", 5.0)
        )
        self.min_valid_depth_m = float(
            rospy.get_param("~min_valid_depth_m", 0.05)
        )
        ...
        self.forward_slow_pwm = int(
            rospy.get_param("~forward_slow_pwm", 8)
        )

        # launch에서 개별 pwm 주는 경우 우선 사용
        self.pwm_forward_l = int(
            rospy.get_param("~pwm_forward_l", self.forward_pwm)
        )
        self.pwm_forward_r = int(
            rospy.get_param("~pwm_forward_r", self.forward_pwm)
        )
        self.pwm_turn_left_l = int(
            rospy.get_param("~pwm_turn_left_l", -self.turn_pwm)
        )
        self.pwm_turn_left_r = int(
            rospy.get_param("~pwm_turn_left_r", self.turn_pwm)
        )
        self.pwm_turn_right_l = int(
            rospy.get_param("~pwm_turn_right_l", self.turn_pwm)
        )
        self.pwm_turn_right_r = int(
            rospy.get_param("~pwm_turn_right_r", -self.turn_pwm)
        )
        self.pwm_backward_l = int(
            rospy.get_param("~pwm_backward_l", 0)
        )
        self.pwm_backward_r = int(
            rospy.get_param("~pwm_backward_r", 0)
        )

        self.debug_image_topic = rospy.get_param(
            "~debug_image_topic", "/panel/debug_image"
        )
        self.mode_topic = rospy.get_param(
            "~mode_topic", "/marker_pose_controller/mode"
        )
        ...
        rospy.loginfo(
            ...
            str(self.publish_debug_image_enabled)
        )
        ...
        self.pub_mode = rospy.Publisher(
            self.mode_topic, String, queue_size=1
        )

        self.pub_doc_pwm = rospy.Publisher(
            self.doc_pwm_topic, Int16MultiArray, queue_size=1
        )
        self.pub_doc_cmd_char = rospy.Publisher(
            self.char_cmd_topic, String, queue_size=1
        )
        self.pub_done = rospy.Publisher(
            self.topic_done, Int32, queue_size=1, latch=True
        )
        self.pub_docking_done = rospy.Publisher(
            self.docking_done_topic, Bool, queue_size=1
        )
    ...
    def color_cb(self, msg):
        ...
        try:
            ...
            rospy.logerr_throttle(
                1.0,
                "color_cb bridge error: %s",
                str(e)
            )
            return

        # 디버그 화면 또는 디버그 이미지 publish가 필요할 때만
        # 프레임 복사와 중심선 그리기를 수행한다.
        annotated = None
        ...
        if not self.enabled:
            if self.debug_visualization_enabled:
                ...
                if self.view_image:
                    cv2.imshow(
                        "marker_pose_controller_left_panel",
                        annotated
                    )
                    cv2.waitKey(1)
        ...
        if self.center_tolerance_px is not None:
            ...
            center_tol_px = int(
                max(
                    1,
                    abs(self.center_tolerance_ratio) * w
                )
            )
        else:
            center_tol_px = 40
    ...
    def detect_target(self, frame):
        ...
        target_candidates = [
            d
            for d in candidates
            if d["cls_name"] == self.target_class_name
        ]
    ...
    def get_depth_m(self, u, v):
        ...
        if (
            ...
            or depth_m > self.max_valid_depth_m
        ):
            return -1.0
    ...
    def control_approach(
        ...
        if depth_m <= self.stop_distance_m:
            rospy.loginfo_throttle(
                1.0,
                "Target reached. depth=%.3fm",
                depth_m
            )
            self.stop_robot()
            self.publish_mode("DONE")
            self.pub_done.publish(
                Int32(data=self.active_mission)
            )
            self.pub_docking_done.publish(
                Bool(data=True)
            )
            self.enabled = False
            self.active_mission = 0
        ...
        if depth_m > 0.60:
            ...
            slow_l = int(
                np.sign(self.pwm_forward_l)
                * min(
                    abs(self.pwm_forward_l),
                    abs(self.forward_slow_pwm)
                )
            )
            slow_r = int(
                np.sign(self.pwm_forward_r)
                * min(
                    abs(self.pwm_forward_r),
                    abs(self.forward_slow_pwm)
                )
            )
    ...
    def publish_debug_image(self, annotated, header):
        ...
        try:
            ...
            rospy.logerr_throttle(
                1.0,
                "debug image publish error: %s",
                str(e)
            )

    def publish_mode(self, mode_str):
        self.mode = mode_str
        self.pub_mode.publish(
            String(data=mode_str)
        )
