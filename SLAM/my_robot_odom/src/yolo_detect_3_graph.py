...
class YoloObjectTFDetector:
    def __init__(self):
        ...
        self.contact_q3_delta_th = 30.0
        self.contact_q2_abs_delta_th = 40.0
        self.contact_consecutive_required = 1
        self.pose_push_timeout = 6.0

        # current trace / graph
        self.current_trace_dir = os.path.expanduser("~/Downloads/graph")
        self.current_trace_post_contact_duration = 3.0
        self.q3_current_threshold = 300.0
        self.current_trace_active = False
        self.current_trace_label = ""
        self.current_trace_data = []
        self.current_trace_start_wall = None
        self.current_trace_push_start_rel = None
        self.current_trace_contact_rel = None
        ...
        os.makedirs(self.current_trace_dir, exist_ok=True)
    ...
    def current_callback(self, msg):
        try:
            ...
            if self.current_trace_active and self.current_trace_start_wall is not None:
                data = list(msg.data)
                if len(data) > max(self.current_idx_q2, self.current_idx_q3):
                    rel_t = time.time() - self.current_trace_start_wall
                    self.current_trace_data.append({
                        "t": float(rel_t),
                        "q2": float(data[self.current_idx_q2]),
                        "q3": float(data[self.current_idx_q3]),
                    })
        except Exception:
            self.latest_joint_current_raw = None
    ...
    def start_current_trace(self, label="target"):
        self.current_trace_active = True
        self.current_trace_label = str(label)
        self.current_trace_data = []
        self.current_trace_start_wall = time.time()
        self.current_trace_push_start_rel = None
        self.current_trace_contact_rel = None
        rospy.loginfo("Current trace started [%s]", label)

    def mark_current_trace_push_start(self):
        if self.current_trace_active and self.current_trace_start_wall is not None:
            self.current_trace_push_start_rel = time.time() - self.current_trace_start_wall
            rospy.loginfo("Current trace push start: %.3f sec", self.current_trace_push_start_rel)

    def mark_current_trace_contact(self):
        if self.current_trace_active and self.current_trace_start_wall is not None and self.current_trace_contact_rel is None:
            self.current_trace_contact_rel = time.time() - self.current_trace_start_wall
            rospy.loginfo("Current trace contact: %.3f sec", self.current_trace_contact_rel)

    
    def save_current_trace_plot(self, label="target", suffix=""):
        if not self.current_trace_data:
            rospy.logwarn("No current trace data to save for [%s]", label)
            self.current_trace_active = False
            return None

        times = np.array([d["t"] for d in self.current_trace_data], dtype=np.float64)
        q3 = np.array([d["q3"] for d in self.current_trace_data], dtype=np.float64)

        if len(q3) >= 2:
            dq3 = np.diff(q3, prepend=q3[0])
            dq3[0] = 0.0
        else:
        ...
        suffix_txt = ("_" + suffix) if suffix else ""
        save_path = os.path.join(self.current_trace_dir, f"{safe_label}_{stamp}{suffix_txt}.png")

        fig, axes = plt.subplots(1, 2, figsize=(16, 7), sharex=False)

        axes[0].plot(times, q3, label="q3 current")
        axes[0].axhline(self.q3_current_threshold, linestyle="--", linewidth=1.5,
                        label=f"q3 current threshold={self.q3_current_threshold:.1f}")
        axes[0].set_xlabel("Time [s]")
        axes[0].set_ylabel("Current")
        axes[0].set_title(f"q3 current [{label}]")
        axes[0].grid(True)
        axes[0].legend(loc="best")

        axes[1].plot(times, dq3, label="q3 delta = current[n]-current[n-1]")
        axes[1].axhline(self.contact_q3_delta_th, linestyle="--", linewidth=1.5,
                        label=f"+threshold={self.contact_q3_delta_th:.1f}")
        axes[1].set_xlabel("Time [s]")
        axes[1].set_ylabel("Delta current")
        axes[1].set_title("q3 delta")
        axes[1].grid(True)
        ...
        rospy.loginfo("Saved current trace plot [%s]: %s", label, save_path)
        self.current_trace_active = False
        return save_path
    ...
    def execute_pose_push(self, label="target", distance_y=None):
        ...
        try:
            ...
            self.start_current_trace(label=label)
            baseline = self.measure_push_current_baseline(label=label)
            if baseline is None:
                self.save_current_trace_plot(label=label, suffix="baseline_failed")
                return False
            ...
            if not plan_success or traj is None:
                ...
                self.save_current_trace_plot(label=label, suffix="plan_failed")
                return False

            self.mark_current_trace_push_start()
            ...
            while not rospy.is_shutdown():
                q2, q3 = self.get_current_q2_q3_raw()
                if q3 is not None and prev_q3 is not None:
                    dq3 = q3 - prev_q3
                    rospy.loginfo_throttle(0.1, "q3 immediate delta [%s]: q3=%.1f dq3=%.1f threshold=%.1f",
                                           label, q3, dq3, self.contact_q3_delta_th)
                    if dq3 >= self.contact_q3_delta_th:
                        contact_count += 1
                    else:
                        contact_count = 0
                    ...
                    if contact_count >= self.contact_consecutive_required:
                        ...
                        self.mark_current_trace_contact()
                        rospy.loginfo("Immediate stop by q3 delta threshold [%s]: dq3=%.1f >= %.1f",
                                      label, dq3, self.contact_q3_delta_th)
                        self.arm_group.stop()
                        break
                ...
                if time.time() - start_t > self.pose_push_timeout:
                    rospy.logwarn("Pose push timeout for [%s]", label)
                    self.arm_group.stop()
                    break
            ...
            if contact_triggered:
                rospy.sleep(self.current_trace_post_contact_duration)
                self.save_current_trace_plot(label=label, suffix="contact_immediate_stop")
                self.move_to_saved_pre_push_then_start_pose(label=label)
            else:
                self.save_current_trace_plot(label=label, suffix="no_contact")
            return True
        except Exception as e:
            rospy.logerr("execute_pose_push failed for [%s]: %s", label, str(e))
            try:
                self.save_current_trace_plot(label=label, suffix="exception")
            except Exception:
                pass
            return False
            ...
            if contact_triggered:
                rospy.sleep(self.current_trace_post_contact_duration)
                self.save_current_trace_plot(label=label, suffix="contact")
                self.move_to_saved_pre_push_then_start_pose(label=label)
            else:
                self.save_current_trace_plot(label=label, suffix="no_contact")
            return True
    ...
    def run(self):
        ...
        while not rospy.is_shutdown():
            try:
                ...
                detect_result = self.detect_model.predict(source=color_image, conf=self.detect_conf_thresh,
                                                          iou=self.detect_iou_thresh, verbose=False)[0]
                ...
                if detect_result is not None and detect_result.boxes is not None and len(detect_result.boxes) > 0:
                    ...
                    for box in detect_result.boxes:
                        ...
                        z = self.get_depth_robust(depth_frame, cx, cy, r=self.depth_patch_radius)
                        if z <= 0.0:
                            continue

                        point_3d = rs.rs2_deproject_pixel_to_point(self.color_intrinsics, [cx, cy], z)
                        ...
                        text = f"{label} {pressed_label.upper()}({pressed_conf:.2f}) z={Z:.2f}m"
