...
class HybridNavigator:
    ...
    목표 시나리오
      1) goto(index) 로 waypoint 자율주행
         - 수정된 구조:
           goto index 0 -> waypoint 0 주행 -> waypoint 1 주행 -> NAV_REACHED:0 발행
           goto index 1 -> waypoint 2 주행 -> waypoint 3이 없으면 waypoint 2 도착 후 NAV_REACHED:1 발행
           즉, 서비스 index는 waypoint 번호가 아니라 2개 waypoint 묶음의 구간 번호로 사용한다.
    ...
    def __init__(self):
        ...
        # ===== goto 구간 주행용 변수 추가 =====
        # goto index 0 -> waypoint 0, 1
        # goto index 1 -> waypoint 2, 3
        # 마지막 waypoint가 없으면 start waypoint만 주행
        self.chain_goto_active = False
        self.chain_section_index = None
        self.chain_start_index = None
        self.chain_end_index = None
    ...
    def _nav_event_index(self, waypoint_index):
        """
        goto 구간 주행 중이면 서비스로 받은 구간 index를 이벤트 번호로 사용한다.
        일반 start/manual 구조에서는 기존처럼 실제 waypoint index를 사용한다.
        """
        if self.chain_goto_active and self.chain_section_index is not None:
            return int(self.chain_section_index)
    ...
    def _goal_done_cb(self, status, _result):
        with self.lock:
            ...
            if status == GoalStatus.SUCCEEDED:
                ...
                #     waypoint 1까지 이어서 가야 하므로 WAIT_PANEL_CMD로 넘기지 않는다.
                is_chain_intermediate = (
                    self.chain_goto_active and
                    self.chain_end_index is not None and
                    self.current_index < self.chain_end_index
                )

                if is_chain_intermediate:
                    self.state = self.ST_IDLE

                    rospy.loginfo("[hybrid_nav] chain intermediate reached idx=%d name=%s -> continue",
                                  self.current_index, self._wp_name(self.current_index))
                    rospy.loginfo("[hybrid_nav] pending marker mission=%s for waypoint=%s",
                                  str(mission), self._wp_name(self.current_index))

                    # 중간 waypoint 도착은 완료 신호를 보내지 않는다.
                    self._publish_vision_debug_mode("off")
                    return
                ...
                # goto 구간 주행 중이면 waypoint index가 아니라 서비스 index를 발행한다.
                event_index = self._nav_event_index(self.current_index)
                self._publish_event("NAV_REACHED:%d" % event_index)
        ...
        while not rospy.is_shutdown() and not self.stop_requested:
            ...
            while not rospy.is_shutdown() and not self.stop_requested:
                ...
                if (rospy.Time.now() - t0).to_sec() > self.goal_timeout:
                    ...
                    event_index = self._nav_event_index(index)
                    self._publish_event("NAV_FAILED:%d" % event_index)
    ...
    def _run_goto_section_with_retry(self, section_index, start_index, end_index):
        """
        goto 서비스용 구간 주행 함수.

        예:
          section_index=0 -> start_index=0, end_index=1
            waypoint 0 도착 시 완료 이벤트 없음
            waypoint 1 도착 시 NAV_REACHED:0 발행

          section_index=1 -> start_index=2, end_index=2
            waypoint 2 도착 시 NAV_REACHED:1 발행
        """
        with self.lock:
            self.chain_goto_active = True
            self.chain_section_index = int(section_index)
            self.chain_start_index = int(start_index)
            self.chain_end_index = int(end_index)

        rospy.loginfo("[hybrid_nav] goto section start: section=%d start_wp=%d end_wp=%d",
                      int(section_index), int(start_index), int(end_index))

        try:
            idx = int(start_index)
            while not rospy.is_shutdown() and not self.stop_requested and idx <= int(end_index):
                rospy.loginfo("[hybrid_nav] goto section=%d running waypoint idx=%d/%d",
                              int(section_index), idx, int(end_index))

                self._run_nav_with_retry(idx)

                if self.stop_requested:
                    rospy.loginfo("[hybrid_nav] goto section interrupted by stop")
                    return

                if self.last_goal_result_status != GoalStatus.SUCCEEDED:
                    rospy.logwarn("[hybrid_nav] goto section failed: section=%d waypoint=%d",
                                  int(section_index), idx)
                    return

                idx += 1

            rospy.loginfo("[hybrid_nav] goto section finished: section=%d", int(section_index))
    ...
    def _srv_goto(self, req):
        section_index = int(req.index)

        if section_index < 0:
            return GotoWaypointResponse(success=False, message="Invalid index")

        # ===== goto index를 waypoint index가 아니라 구간 index로 해석 =====
        # index 0 -> waypoint 0, 1
        # index 1 -> waypoint 2, 3
        # 단, 마지막 waypoint가 없으면 start_index만 주행한다.
        start_index = section_index * 2
        end_index = start_index + 1

        if start_index < 0 or start_index >= len(self.wps):
            return GotoWaypointResponse(success=False, message="Invalid index")

        if end_index >= len(self.wps):
        ...
        threading.Thread(
            target=self._run_goto_section_with_retry,
            args=(section_index, start_index, end_index),
            daemon=True
        ).start()

        return GotoWaypointResponse(
            success=True,
            message="Sent goto section #%d: waypoint %d -> %d" % (section_index, start_index, end_index)
        )
