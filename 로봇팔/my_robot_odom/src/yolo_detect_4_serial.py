...
class YoloObjectTFDetector:
    ...
    def print_detected_targets_log(self):
        ...
        rospy.loginfo("========== SERIAL VERSION TARGET LIST ==========")

        for label, info in sorted(self.detected_targets.items()):
            rospy.loginfo(
                "[SERIAL TARGET] %s | base=(%.3f, %.3f, %.3f)m | cam=(%.1f, %.1f, %.1f)mm | conf=%.2f | state=%s(%.2f)",
                label,
    ...
    def execute_pose_push(self, label="target", distance_y=None):
        ...
        try:
            ...
            # 기존 코드: target_pose.position.y += distance_y
            # 문제점: base_link의 +Y 방향으로만 직선 이동함.
            # 수정: 현재 TCP 자세의 local +Y 방향을 base_link 기준 벡터로 변환하여 그 방향으로 이동.
            q = [
            ...
            # TCP local +Y axis expressed in base_link frame
            ...
            try:
                ...
                if detect_result is not None and detect_result.boxes is not None and len(detect_result.boxes) > 0:
                    ...
                    for box in detect_result.boxes:
                        ...
                        cam_x_mm = X * 1000.0
                        text = f"{label} {pressed_label.upper()}({pressed_conf:.2f}) X={cam_x_mm:.1f}mm"
